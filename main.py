...
testing_path = "NguyenAmKiemThu-16k"    # đường dẫn đến file test

def segmentation(audio, frame_length, hop_length, energy_threshold=0.02, zc_rate_threshold=0.1):
...
def Divide(audio):
    length = len(audio)
    part_len = length // 3
    audio = audio[part_len:part_len*2]
    return audio
...
def cal_nfft(audio): #lấy giá trị vector FFT của một audio
    ...
    # frames = frame(audio, frame_length=frame_size, hop_length=hop_size)
    speech_part = segmentation(audio=audio,frame_length=frame_size,hop_length=hop_size)
    middle_part = TrimPart(speech_part)
    fft_frames = np.fft.fft(middle_part,NFFT, axis=0) # hiện tại một cột là một vector
    res = fft_frames
